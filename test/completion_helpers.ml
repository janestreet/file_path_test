...
include Completion_helpers_intf.Definitions
module Unix = Core_unix

let populate paths =
  List.iter paths ~f:(fun path ->
    if String.is_suffix path ~suffix:"/"
    then Filesystem_core.mkdir ~parents:true ?/path
    else (
      Filesystem_core.mkdir ~parents:true ?/(Filename.dirname path);
      Filesystem_core.write_file ?/path ~contents:""))
;;
...
(* We want [output_of f] to run [f ()] and return its output to [stdout].
   [expect_test_output ()] returns all pending output for the current expect test. So if
   we just run [f (); expect_test_output ()], we'll grab not just [f ()]'s output, but
   also everything that came before it.

   [with_empty_expect_test_output] wraps its argument as if the output were just
   beginning. Calls to [expect_test_output] only apply to new output inside that call. So
   in this case, it means we only capture [f ()]'s output, and everyhing else is
   untouched. *)
let output_of f =
...
let completions param arg =
  ...
  output_of (fun () -> Command_test_helpers.complete param ~args:[ arg ])
  |> String.split_lines
  |> List.drop_last_exn
;;

let with_env ~key ~data f =
  let original = Sys.getenv key in
...
let complete_paths path_m param paths ~tmp =
  paths
  |> args_of_paths
  |> List.map ~f:(fun arg -> complete_arg param arg ~tmp, arg)
...
let test_arg_type (type a) (module Path : Path with type t = a) =
  Filesystem_core.within_temp_dir (fun () ->
    let tmp = !/$(Filesystem_core.getcwd ()) in
    populate paths;
    let param =
      let open Command.Param in
      anon ("PATH" %: Path.arg_type)
    in
    complete_paths (module Path) param paths ~tmp)
;;
